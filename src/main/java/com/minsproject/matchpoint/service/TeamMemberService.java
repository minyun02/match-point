...
public class TeamMemberService {
    ...
    public Long create(Long teamId, UserDTO user) {
        Team team = teamRepository.findById(teamId).orElseThrow(() -> new MatchPointException(ErrorCode.TEAM_NOT_FOUND));
        if (team.getStatus() == TeamStatus.PAUSED) {
            throw new MatchPointException(ErrorCode.TEAM_NOT_ACCEPTING_MEMBER);
        }

        List<TeamMember> teamMembers = team.getTeamMembers();
        boolean isUserInTeam = teamMembers.stream().anyMatch(member -> Objects.equals(member.getUser().getId(), user.getUserId()));
        if (isUserInTeam) {
            throw new MatchPointException(ErrorCode.ALREADY_IN_TEAM);
        }
    ...
    public TeamMemberResponse modify(TeamMemberDTO teamMemberDTO, UserDTO userDTO) {
        TeamMember teamMember = teamMemberRepository.findById(teamMemberDTO.getTeamMemberId()).orElseThrow(() -> new MatchPointException(ErrorCode.TEAM_MEMBER_NOT_FOUND));
    ...
    public void delete(Long teamMemberId, UserDTO userDTO) {
        TeamMember teamMember = teamMemberRepository.findById(teamMemberId).orElseThrow(() -> new MatchPointException(ErrorCode.TEAM_MEMBER_NOT_FOUND));
